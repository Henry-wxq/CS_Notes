...
def train_mle_estimator(train_images, train_labels):
    ...
    sum_l = np.sum(train_labels, axis=0)

    theta_mle = np.dot(train_images.T, train_labels) / sum_l
    pi_mle = sum_l / train_labels.shape[0]
...
def train_map_estimator(train_images, train_labels):
    ...
    theta_map = (np.dot(train_images.T, train_labels) + 2) / (sum_l + 4)
    pi_map = sum_l / train_labels.shape[0]
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    log_pi = np.log(pi)
    log_pixel = images @ np.log(theta) + (1 - images) @ np.log(1 - theta)

    sum_log_probs = log_pi + log_pixel

    log_like = sum_log_probs - np.log(np.sum(np.exp(sum_log_probs), axis=1)).reshape((images.shape[0], 1))
...
def accuracy(log_like, labels):
    ...
    # YOU NEED TO WRITE THIS PART
    acc = np.sum(predict(log_like) == np.argmax(labels, axis=1)) / len(labels)
