...
def train_mle_estimator(train_images, train_labels):
    ...
    theta_mle = np.transpose(train_images) @ train_labels / sum_labels
...
def train_map_estimator(train_images, train_labels):
    ...
    theta_map = (np.transpose(train_images) @ train_labels + 2) / (sum_labels + 4)
    pi_map = sum_labels / train_labels.shape[0]
...
def log_likelihood(images, theta, pi):
    ...
    log_pixel = images @ np.log(theta) + (np.ones(images.shape) - images) @ np.log(np.ones(theta.shape) - theta)

    sum = log_pi + log_pixel

    log_like = sum - np.log(np.sum(np.exp(sum), axis=1)).reshape((images.shape[0], 1))
