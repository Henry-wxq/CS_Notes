...
class Employee:
    ...
    def pay(self, pay_date: date) -> None:
        ...
        self.total += self.get_monthly_payment()
...
class HourlyEmployee(Employee):
    ...
    def get_monthly_payment(self) -> float:
        ...
        62.5
        """
        self.total = round(self.hours_per_month * self.hourly_wage, 2)
        return self.total
...
class Company:
    ...
    def total_payroll(self) -> float:
        ...
        count = 0.0
        for employee in self.employees:
            if type(employee) == SalariedEmployee:
                count += employee.total_pay()
            elif type(employee) == HourlyEmployee:
                count += employee.get_monthly_payment()
        return count
