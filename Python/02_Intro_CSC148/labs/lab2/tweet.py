...
class User:
    ...
    def verbosity(self, y: int) -> int:
        ...
        0
        """
        # create a variable with int 0
        # loop the tweets list
            # if the year is required, then get the len of that Tweet and add to
            # the variable
        a = 0
    ...
    def hack(self) -> None:
        ...
        'mwahahaha'
        """
        # loop all of the users that self follows
        for users in self.follows:
            # loop all of the tweets for each user
            for t in users.tweets:
                # change the users.content into 'mwahahaha'
                t.edit('mwahahaha')
