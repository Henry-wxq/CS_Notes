...
def mergesort3(lst: list) -> list:
    ...
    if len(lst) < 2:  # We've provided the base case for you.
        ...
        return [min(c_lst), max(c_lst)]
    else:
        # Divide the list into three parts, and sort them recursively
        triple_num = len(lst) // 3
        left_lst = mergesort3(lst[:triple_num])
        mid_lst = mergesort3(lst[triple_num:triple_num * 2])
        right_lst = mergesort3(lst[triple_num * 2:])

        # use the below function to sort
        return merge3(left_lst, mid_lst, right_lst)
...
def merge3(lst1: list, lst2: list, lst3: list) -> list:
    ...
    separately.
    """
    # Simulate the structure of _merge
    index1 = 0
    index2 = 0
    index3 = 0
    merged = []
    while index1 < len(lst1) and index2 < len(lst2) and index3 < len(lst3):
        # when the number is the smallest at this moment, then append
        if lst1[index1] <= lst2[index2] and lst1[index1] <= lst3[index3]:
            ...
            merged.append(lst2[index2])
            index2 += 1
        elif lst3[index3] <= lst1[index1] and lst3[index3] <= lst2[index2]:
            merged.append(lst3[index3])
            index3 += 1

    # it might exist the situation not satisfying the while loop but still left
    # numbers in the list
    if index1 == len(lst1):
        add1 = lst2[index2:]
        add2 = lst3[index3:]
