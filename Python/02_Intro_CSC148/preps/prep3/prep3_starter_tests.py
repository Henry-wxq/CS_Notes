...
################################################################################
def test_zero_salary_for_Salaried() -> None:
    e = SalariedEmployee(16, 'Chloe', 0.0)
...
def test_zero_working_Hourly() -> None:
    e = HourlyEmployee(17, 'Noble', 10000.0, 0.0)
    e.pay(date(2018, 6, 28))
    e.pay(date(2018, 7, 28))
    e.pay(date(2018, 8, 28))
    assert e.total_pay() == 0.0
...
def test_total_without_date() -> None:
    my_corp = Company([SalariedEmployee(16, 'Chloe', 500.0),
                       HourlyEmployee(17, 'Noble', 100.0, 3.0)])
    assert my_corp.total_payroll() == 0.0


def test_total_severl_month() -> None:
    my_corp = Company([SalariedEmployee(16, 'Chloe', 1200.0),
                       HourlyEmployee(17, 'Noble', 100.0, 3.0)])
    my_corp.pay_all(date(2018, 6, 28))
    my_corp.pay_all(date(2018, 8, 28))
    assert my_corp.total_payroll() == 800.0
...
if __name__ == '__main__':
    import pytest
    pytest.main(['-vs', 'prep3_starter_tests.py'])
