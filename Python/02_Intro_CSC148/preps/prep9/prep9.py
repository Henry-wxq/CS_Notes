...
class BinarySearchTree:
    ...
    def count(self, item: Any) -> int:
        ...
        if self.__contains__(item):
            if self._root == item:
                num += 1
                num += self._left.count(item)
                num += self._right.count(item)
    ...
    def items(self) -> list:
        ...
        if self.is_empty():
            return item_lst
        item_lst.append(self._root)
        item_lst = self._left.items() + item_lst
        item_lst.extend(self._right.items())
        return item_lst
    ...
    def smaller(self, item: Any) -> list:
        ...
        if self._root >= item:
            small_lst = self._left.smaller(item) + small_lst
        elif self._root < item:
            small_lst.append(self._root)
            small_lst.extend(self._right.smaller(item))
            small_lst = self._left.items() + small_lst
        return small_lst
