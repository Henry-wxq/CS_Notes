...
class BinarySearchTree:
    ...
    def count(self, item: Any) -> int:
        ...
        if self.is_empty():
            ...
            if self._root == item:
                item_num += 1
                item_num += self._left.count(item)
                item_num += self._right.count(item)
    ...
    def items(self) -> list:
        ...
        if self.is_empty():
            ...
            r_lst.append(self._root)
            r_lst = self._left.items() + r_lst
            r_lst.extend(self._right.items())
        return r_lst
    ...
    def smaller(self, item: Any) -> list:
        ...
        if self.is_empty():
            ...
            if self._root < item:
                r_lst.append(self._root)
                r_lst += self._right.smaller(item)
                r_lst = self._left.items() + r_lst
            elif self._root >= item:
                r_lst = self._left.smaller(item) + r_lst
            return r_lst
