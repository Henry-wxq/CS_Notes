    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        for i in range(len(answers) - 1):
            for j in range(i + 1, len(answers)):
                if answers[i].is_valid(question) \
                        and answers[j].is_valid(question):
                    similarity_scores. \
                        append(question.get_similarity(answers[i], answers[j]))
                else:
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        if len(answers) < 2:
            if len(answers) == 1 and answers[0].is_valid(question):
                ...
                raise InvalidAnswerError
        return 1.0 - HomogeneousCriterion().score_answers(question, answers)
...
class LonelyMemberCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        for every_answer in all_answer:
            if all_answer.count(every_answer) != len(all_answer) \
                    and all_answer.count(every_answer) == 1:
                return 0.0
        return 1.0
