...
class Instructor:
    """A class storing information of a Instructor in the gym
    ...
    _certificates: All certificates that an instructor owns
    """
    name: str
    _id: int
    _certificates: list[str]

    def __init__(self, set_id: int, name: str) -> None:
        """Initialize an instructor with input set_id and name"""
        self.name = name
        self._id = set_id
        self._certificates = []
    ...
    def add_certificate(self, new_certificate: str) -> bool:
        """Add a new certificate to the instructor iff the certificate does not
        already exist
        ...
        >>> instructor = Instructor(1, 'Henry')
        >>> instructor.add_certificate('Pilates')
        True
        """
    ...
    def get_certificates(self) -> list:
        """Return the list that the instructor has already owned

        >>> instructor = Instructor(1, 'Henry')
        >>> instructor.add_certificate('Pilates')
        True
        >>> instructor.get_certificates()
        ['Pilates']
        """
...
class Gym:
    ...
    def add_instructor(self, instructor: Instructor) -> bool:
        ...
        True
        """
        new_id = instructor.get_id()
        if new_id not in self._instructors:
            self._instructors[new_id] = instructor
            return True
        return False
    ...
    def add_workout_class(self, workout_class: WorkoutClass) -> bool:
        ...
        True
        """
        new_wc = workout_class.name
        if new_wc not in self._workouts:
            self._workouts[new_wc] = workout_class
            return True
        return False
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        True
        """
        had_c = self._instructors[instr_id].get_certificates()
        required_c = self._workouts[workout_name].get_required_certificates()
        for c in required_c:
            if c not in had_c:
                return False
        if time_point not in self._schedule:
            ...
            room_n = self._schedule[time_point].keys()
            for time in self._schedule:
                if time == time_point and room_name in room_n:
                    return False
            for room in room_n:
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        c_r = ''
        c_num = 0
        for room in self._schedule[time_point].keys():
            c_name_lst = self._schedule[time_point][room][2]
            if client in c_name_lst:
                return False
        for room in self._schedule[time_point].keys():
            c_name_lst = self._schedule[time_point][room][2]
            if self._schedule[time_point][room][1].name == \
                    workout_name and c_num <= len(c_name_lst) < \
                    self._room_capacities[room]:
                c_num = len(c_name_lst)
                c_r = room
        if c_r != '':
            self._schedule[time_point][c_r][2].append(client)
            return True
        else:
            return False
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        """
        pay_lst = []
        work_time = self.instructor_hours(time1, time2)
        for id_ins in sorted(work_time.keys()):
            ins = self._instructors[id_ins]
            num_c = len(ins.get_certificates())
            money = work_time[id_ins] * base_rate + \
                work_time[id_ins] * num_c * BONUS_RATE
            pay_lst.append((id_ins, ins.name,
                            work_time[id_ins], money))
        return pay_lst
    ...
    def _is_instructor_name_unique(self, instructor: Instructor) -> bool:
        ...
        """
        a = 0
        for id_ins in self._instructors:
            if instructor.name == self._instructors[id_ins].name:
                a = a + 1
        if a <= 1:
            return True
        return False
    ...
    def offerings_at(self, time_point: datetime) -> list[dict[str, str | int]]:
        ...
        if time_point in self._schedule:
            r_m = self._schedule[time_point]
            for room in sorted(r_m.keys(), key=str.lower):
                ...
                ins = r_m[room][0]
                ins_n = ins.name
                a = self._room_capacities[room] - r
                if not self._is_instructor_name_unique(ins):
                    id_t = ins.get_id()
                    ins_n += (' (' + str(id_t) + ')')
                off_lst.append(create_offering_dict(date, time, c_n, room, r,
                                                    a, ins_n))
        return off_lst
    ...
    def to_schedule_list(self, week: datetime = None) \
        ...
        """
        s_lst = []
        for time_points in sorted(self._schedule.keys()):
            if in_week(time_points, week):
                s_lst.extend(self.offerings_at(time_points))
        return s_lst
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (self.name == other.name
                ...
                and self._room_capacities == self._room_capacities)
